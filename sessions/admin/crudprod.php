                   ...
                   $result = $conn->query($sql); // $conn from db_connect.php
                    ...
                    if($result->num_rows> 0): // object oriented style
                        ...
                        foreach($rows as $row) {
                            ...
                            echo "<p><div>{$row['name']}</div> <div><a href='updateproduct.php?id={$row['id']}' class='btn btn-kommerz'>update</a> <a href='deleteproduct.php?id={$row['id']}' class='btn btn-danger'>delete</a></div></p>";
